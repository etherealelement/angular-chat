...
export class AuthService {
    ...
    login(payload: LoginAuthPayload): void {
        ...
        this.httpClient
            .post<LoginAuthResponse>(`${this.baseApiUrl}token`, formData)
            .pipe(tap((res: LoginAuthResponse) => this.saveTokens(res)))
            .subscribe({
                ...
                error: (error: any) => {
                    this.errorText.set(
                        error.error.detail || 'Что-то пошло не так',
                    );
                    console.log(error);
                },
            });
    ...
    refreshAuthToken(): Observable<LoginAuthResponse> {
        return this.httpClient
            .post<LoginAuthResponse>(`${this.baseApiUrl}refresh`, {
